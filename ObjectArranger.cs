...
public class ObjectArranger : MonoBehaviour {
    ...
    // Object is the midpoint of the circle
    [System.Serializable]
            ...
            case Arrangement.LINE:
                for (int i = 0; i < lineParameters.numberOfObjects; i++) {
                    ...
                    if (lineParameters.length == 0)
                        pos = lineParameters.spacing * i;
                    else
                        pos = lineParameters.length / lineParameters.numberOfObjects * i;

                    if (lineParameters.horizontalInsteadOfVertical)
                        position.z = pos;
                    else
                        position.y = pos;
