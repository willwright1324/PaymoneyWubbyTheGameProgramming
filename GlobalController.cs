    ...
    private void Update() {
        ...
        if (Input.GetButtonDown("Cancel") && !menuActive && canPause) {
            if (isPaused) {
                ...
                GameObject.Find("PauseUI/KeyTotalText").GetComponent<Text>().text = "Total Keys: " + gameData.totalKeys + "/" + ItemManagerScript.itemList[ItemManagerScript.GetItemIndex("StreamKey")].itemGameTotal;
                Time.timeScale = 0;
                isPaused = true;
            }
        }
